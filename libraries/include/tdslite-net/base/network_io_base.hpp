...
namespace tdsl {
    ...
    namespace net {
        ...
        public:
            ...
            tdsl::uint32_t do_receive_tds_pdu() noexcept {
                ...
                do {
                    ...
                    // level.
                    const auto hdr_recv_result =
                        impl().do_recv(tds_hbuf_s.size_bytes(), tds_hbuf_s);
                    if (!hdr_recv_result) {
                        TDSL_DEBUG_PRINTLN(
                            "network_io_base::do_receive_tds_pdu(...) -> failed to read TDS "
                            "header (%d)",
                            hdr_recv_result.error());
                    ...
                    if (length < sizeof(detail::tds_header) || length > k_max_length) {
                        TDSL_DEBUG_PRINTLN("invalid tds message length %u (type=%u)",
                                           length, static_cast<unsigned>(message_type));
                        return processed_tds_message_count;
                    }
                    ...
                    if (packet_data_size > network_buffer_remaining_bytes()) {
                        TDSL_DEBUG_PRINTLN("Cannot fit complete message into network "
                                           ...
                                           packet_data_size,
                                           network_buffer_remaining_bytes());
                        do {
                            if (network_buffer_remaining_bytes() == 0) {
                                ...
                                network_buffer_reset();
                                return processed_tds_message_count;
                            }
                            const auto recv_result =
                                impl().do_recv(network_buffer_remaining_bytes());
                            if (recv_result) {
                        ...
                        } while (packet_data_size > network_buffer_remaining_bytes());
                    }
                    else {
        ...
        private:
            inline tdsl::size_t network_buffer_remaining_bytes() noexcept {
                auto writer = network_buffer.get_writer();
                return writer->remaining_bytes();
            }

            inline void network_buffer_reset() noexcept {
                auto writer = network_buffer.get_writer();
                writer->reset();
            }
