...
namespace tdsl { namespace detail {
    ...
    public:
        ...
        /**
         * MS-TDS PRELOGIN exchange required by SQL Server 2019+ before LOGIN7.
         * tdslite upstream sends LOGIN directly; modern servers may close TCP without this.
         */
        inline bool exchange_prelogin() noexcept {
            static const tdsl::uint8_t prelogin_body[] = {
                0x00, 0x00, 0x0F, 0x00, 0x06,                   // VERSION -> off 15, len 6
                0x01, 0x00, 0x15, 0x00, 0x01,                   // ENCRYPTION -> off 21, len 1
                0xFF, 0x00, 0x00, 0x00, 0x00,                   // TERMINATOR
                0x00, 0x00, 0x00, 0x00, 0x00, 0x00,             // client version (driver)
...

            {
                ...
                writer->reset();
                (void)writer->write(tdsl::byte_view{prelogin_body, sizeof(prelogin_body)});
            }
            this->send_tds_pdu(e_tds_message_type::pre_login);

            tdsl::uint8_t hdr[sizeof(detail::tds_header)] = {};
            const auto hdr_result = this->do_recv(sizeof(hdr), tdsl::byte_span{hdr});
            if (!hdr_result) {
                TDSL_DEBUG_PRINTLN("prelogin: failed reading response header (%d)",
                                   hdr_result.error());
                return false;
            }

            const tdsl::uint16_t packet_length =
                static_cast<tdsl::uint16_t>((static_cast<tdsl::uint16_t>(hdr[2]) << 8) | hdr[3]);
            if (packet_length < sizeof(detail::tds_header)) {
                TDSL_DEBUG_PRINTLN("prelogin: invalid response length %u", packet_length);
                return false;
            }

            const tdsl::uint16_t body_length = packet_length - sizeof(detail::tds_header);
            tdsl::uint8_t body[512]          = {};
            if (body_length > sizeof(body)) {
                TDSL_DEBUG_PRINTLN("prelogin: response too large (%u)", body_length);
                return false;
            }

            const auto body_result = this->do_recv(body_length, tdsl::byte_span{body, body_length});
            if (!body_result) {
                TDSL_DEBUG_PRINTLN("prelogin: failed reading response body (%d)",
                                   body_result.error());
                return false;
            }

            tdsl::uint8_t negotiated_encrypt = 0xFF;
            for (tdsl::uint16_t i = 0; (i + 5) <= body_length;) {
                const tdsl::uint8_t token = body[i];
                if (token == 0xFF) {
                    break;
                }

                const tdsl::uint16_t offset = static_cast<tdsl::uint16_t>(
                    (static_cast<tdsl::uint16_t>(body[i + 1]) << 8) | body[i + 2]);
                const tdsl::uint16_t length = static_cast<tdsl::uint16_t>(
                    (static_cast<tdsl::uint16_t>(body[i + 3]) << 8) | body[i + 4]);

                if (token == 0x01 && length >= 1 && (offset + length) <= body_length) {
                    negotiated_encrypt = body[offset];
                }

                i += 5;
            }

            TDSL_DEBUG_PRINTLN("prelogin: server encryption option=%u", negotiated_encrypt);

            if (negotiated_encrypt == 0x01 || negotiated_encrypt == 0x03) {
                TDSL_DEBUG_PRINTLN("prelogin: server requires TLS (not supported by tdslite)");
                return false;
            }
