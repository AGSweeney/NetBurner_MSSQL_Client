...
namespace tdsl {
    ...
    template <>
    inline TDSL_NODISCARD constexpr auto byte_swap<char>(char v) noexcept -> char {
        return v;
    }
    ...
    template <>
    inline TDSL_NODISCARD constexpr auto byte_swap<char16_t>(char16_t v) noexcept -> char16_t {
        return static_cast<char16_t>(__builtin_bswap16(static_cast<tdsl::uint16_t>(v)));
    }
    ...
    template <>
    inline TDSL_NODISCARD auto byte_swap<float>(float v) noexcept -> float {
        tdsl::uint32_t raw{};
    ...
    template <>
    inline TDSL_NODISCARD auto byte_swap<double>(double v) noexcept -> double {
        tdsl::uint64_t raw{};
        __builtin_memcpy(&raw, &v, sizeof(raw));
